import { Unit, UnitId, Position, Milliseconds } from '../../types'
...
const computePathTo = (unit: Unit, destination: Destination, tolerance: number): boolean => {
...
function moveTowards(unit: Unit, destination: Destination, tolerance: number, dt: Milliseconds): MoveResult {
    ...
    if (!unit.pathToNext) {
        if (!computePathTo(unit, destination, tolerance))
            return 'Unreachable';
    } else {
...
export function moveTowardsPoint(unit: Unit, targetPos: Position, tolerance: number, dt: Milliseconds): MoveResult {
    return moveTowards(unit, {type: 'MapDestination', position: {x: targetPos.x, y: targetPos.y}}, MAP_MOVEMENT_TOLERANCE, dt);
}

export function moveTowardsUnit(unit: Unit, target: Unit, extraTolerance: number, dt: Milliseconds): MoveResult {
    const bc = getBuildingComponent(target);

    const tolerance = extraTolerance + UNIT_FOLLOW_DISTANCE;
    if (!bc) {
        return moveTowardsPoint(unit, getUnitReferencePosition(target), tolerance, dt);
    } else {
        const tiles = tilesTakenByBuilding(bc, target.position);
        return moveTowards(unit, { type: 'BuildingDestination', tiles }, tolerance, dt);
    }
}

export const moveTowardsUnitById = (unit: Unit, targetId: UnitId, extraTolerance: number, dt: Milliseconds): MoveToUnitResult => {
    const target = g.units.find(u => u.id === targetId);
    if (target) {
        return moveTowardsUnit(unit, target, extraTolerance, dt);
    } else {
...
export function moveToPointOrCancelCommand(unit: Unit, p: Position, dt: Milliseconds) {
    switch (moveTowardsPoint(unit, p, MAP_MOVEMENT_TOLERANCE, dt)) {
        case 'Moving':
            unit.state.action = 'Moving';
            break;
        case 'ReachedTarget':
...
const moveApply = (unit: Unit, mc: Mover, dt: Milliseconds) => {
